...
if __name__=="__main__":
     ...
     model = Model(inputs=input, outputs=[window_output])   
     model.summary()
     training_generator=IEMR()
     valid_generator=IEMR_valid()
     
     earlyStopping = EarlyStopping(monitor='loss', patience=20, verbose=0, mode='min')
     mcp_save = ModelCheckpoint('./06020516_dropout.hdf5', save_best_only=True, monitor='loss', mode='min')
     ...
     model.fit_generator(training_generator,steps_per_epoch=1000,epochs=300,validation_data=valid_generator,validation_steps=1000,callbacks=[earlyStopping, mcp_save, reduce_lr_loss,history,tensorboard])
