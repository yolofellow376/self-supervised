...
class IEMR(Sequence):
   ...
   ''' def __getitem__(self, idx):
        ...
        for i in range batch_size:
          x,y=self.__get_one_item__(self,idx + i))
          batch_x.append(x)
          batch_y.append(y)
          batch_x=tf.convert_to_tensor(batch_x)
          batch_y=tf.convert_to_tensor(batch_y)
        return batch_x, batch_y
    def __get__one_item__(self,index):
   '''             
    def __getitem__(self, index):
        ...
        #labels=np.expand_dims(labels, axis=0)
        intensity_arr=intensity_arr.astype('float32')
        intensity_arr=tf.convert_to_tensor(intensity_arr)
        labels=tf.convert_to_tensor(labels)
        
        
        index_label=np.zeros(30)
        index_label[labels[0][0]]=1
        signal_label=np.zeros(30)
        signal_label[labels[0][1]]=1
       #print("signal_label:",signal_label)
        ...
        c_labels = tf.concat([index_label, signal_label],0)
        c_labels = tf.expand_dims(c_labels,axis=0)
        a_labels=[]
        a_labels.append(index_label)
        a_labels.append(signal_label)
        a_labels=tf.convert_to_tensor(c_labels)
        #c_labels = tf.expand_dims(c_labels,axis=0)
        #print("arr:",tf.shape(intensity_arr), "\n conca:",tf.shape(c_labels),tf.shape(a_labels),"\nlabels:",tf.shape(index_label),tf.shape(signal_label))
        #print(c_labels)
